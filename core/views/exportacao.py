...
from ..models import (Aluno, AnoLetivo, ConteudoProgramatico, Disciplina,
                     ...
                     ProvaAuxiliar, RecuperacaoFinal, ConselhoClasse, GrupoDisciplina)
from ..services.calculo_notas import (carregar_dados_boletim_aluno, calcular_notas_disciplina, round_2)
...
def exportar_boletim_turma_pdf(request, codigo):
    prof = get_professor(request.user)
    pode_exportar = request.user.is_superuser or (prof and prof.pode_acessar_modulo_notas)
    ...
    pas_map = {(p.aluno_id, p.disciplina_id, p.numero_pa): p.nota for p in pas_turma_qs}
    ...
    elems.append(Paragraph('NA = Não Avaliado | * = Substituído/recuperado por PA | (CC) = Promovido pelo Conselho | Nota mínima: 5,00', subtitle_style))
    elems.append(Spacer(1, 0.8*cm))

    # Construir tabela
    header = ['Aluno']
    for d in disciplinas:
        header.append(d.nome[:15] + ('...' if len(d.nome) > 15 else ''))
    ...
    for aluno in alunos:
        ...
        for disc in disciplinas:
            ...
            for b_key in ('b1', 'b2', 'b3', 'b4'):
                val_info = res[b_key]
                if val_info['valor'] is not None:
                    s = f"{val_info['valor']:.2f}"
                    if val_info['substituido_por_pa']:
                        s += '*'
                    cel_str.append(s)
                else:
                    cel_str.append('-')
            
            mf = res['media_final'] if res['media_final'] is not None else res['media_anual']
            if mf is not None:
                mf_str = f"{mf:.2f}"
                if res['promovido_conselho']:
                    mf_str += ' (CC)'
                cel_str.append(mf_str)
                medias_finais_aluno.append(mf)
            else:
                cel_str.append('-')
            
            linha.append(" | ".join(cel_str))
            
        mg = round_2(sum(medias_finais_aluno) / Decimal(str(len(medias_finais_aluno)))) if medias_finais_aluno else None
        linha.append(f"{mg:.2f}" if mg is not None else '-')
        data.append(linha)

    # Estilizar tabela
    col_widths = [5.5*cm] + [4.0*cm for _ in disciplinas] + [2.2*cm]
    ...
    doc.build(elems, onFirstPage=_add_signature_footer, onLaterPages=_add_signature_footer)
    return _pdf_response(buf, f'boletim_consolidado_turma_{turma.codigo}.pdf')
...
def exportar_boletim_aluno_pdf(request, pk):
    ...
    # Cabeçalho
    title_style = _get_saas_title_style(styles)
    elems.append(Paragraph('Boletim Escolar Individual', title_style))
    elems.append(Spacer(1, 0.3*cm))
    
    info_style = ParagraphStyle(
        'Info', parent=styles['Normal'],
        fontSize=11, alignment=1, textColor=colors.HexColor('#333333')
    )
    elems.append(Paragraph(f'<b>Aluno(a):</b> {aluno.nome} &nbsp;&nbsp;|&nbsp;&nbsp; <b>Turma:</b> {turma.codigo} &nbsp;&nbsp;|&nbsp;&nbsp; <b>Ano Letivo:</b> {request.ano_letivo.ano}', info_style))
    elems.append(Spacer(1, 0.6*cm))

    # Tabela de notas completa
    data = [['Disciplina', '1º B', '2º B', 'PA1', '3º B', '4º B', 'PA2', 'M. Anual', 'Rec. F.', 'M. Final', 'Situação']]
    
    def _fmt(v_dict):
        if not v_dict or v_dict.get('valor') is None:
            return '-'
        v = v_dict['valor']
        s = f"{v:.2f}"
        if v_dict.get('substituido_por_pa'):
            s += '*'
        return s
    ...
    for linha in grupos_no_boletim:
        row = [
            ...
            _fmt(linha['b1']),
            _fmt(linha['b2']),
            f"{linha['pa1']['valor']:.2f}" if linha['pa1']['valor'] is not None else '-',
            _fmt(linha['b3']),
            _fmt(linha['b4']),
            f"{linha['pa2']['valor']:.2f}" if linha['pa2']['valor'] is not None else '-',
            f"{linha['media_anual']:.2f}" if linha['media_anual'] is not None else '-',
            f"{linha['rec_final']['valor']:.2f}" if linha['rec_final']['valor'] is not None else '-',
            (f"{linha['media_final']:.2f}" + (' (CC)' if linha.get('promovido_conselho') else '')) if linha['media_final'] is not None else '-',
            linha.get('situacao', '-'),
        ]
        data.append(row)

    col_widths = [4.6*cm, 1.2*cm, 1.2*cm, 1.1*cm, 1.2*cm, 1.2*cm, 1.1*cm, 1.6*cm, 1.6*cm, 1.6*cm, 2.0*cm]
    ...
    ts = TableStyle([
        ...
        ('GRID', (0, 0), (-1, -1), 0.5, colors.HexColor('#e0e8f5')),
        ('BACKGROUND', (7, 1), (7, -1), colors.HexColor('#eef4ff')),
        ('BACKGROUND', (9, 1), (9, -1), colors.HexColor('#f4fdf8')),
    ])
    ...
    for i, linha in enumerate(grupos_no_boletim, start=1):
        if linha['is_grupo']:
            ...
            ts.add('BACKGROUND', (0, i), (6, i), colors.HexColor('#f9fbff'))

    t.setStyle(ts)
    elems.append(t)
    
    elems.append(Spacer(1, 0.8*cm))
    leg_style = ParagraphStyle('Leg', parent=styles['Normal'], fontSize=8.5, textColor=colors.HexColor('#555555'))
    elems.append(Paragraph('<b>Legenda:</b> * = Substituído/recuperado por Prova Auxiliar &nbsp;|&nbsp; (CC) = Promovido pelo Conselho de Classe', leg_style))
    elems.append(Paragraph('Nota mínima para aprovação: <b>5,00</b> &nbsp;|&nbsp; Em caso de aprovação na Recuperação Final ou Conselho, a Média Final é fixada em 5,00.', leg_style))
