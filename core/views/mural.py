...
def mural_avisos(request):
    ...
    pode_criar = False
    if request.user.is_superuser or (prof and prof.cargo in ('ADMIN', 'DIRETOR', 'COORDENADOR', 'AUX_COORD', 'SECRETARIA')):
        pode_criar = True
...
def mural_criar_aviso(request):
    prof = get_professor(request.user)
    if not request.user.is_superuser and (not prof or prof.cargo not in ('ADMIN', 'DIRETOR', 'COORDENADOR', 'AUX_COORD', 'SECRETARIA')):
